...
fixed_lines = []
for line in lines:
    # Fix line 75: psql command with \" inside double quotes
    if 'command: "psql -d harness -c \\"SELECT' in line:
        line = line.replace('command: "', "command: '")
        line = line.rstrip('\n').rstrip('"') + "'\n"
    # Fix line 78: find command with \| inside double quotes
    elif "command: \"find /opt/harness/src/modules -name '*.py' -exec grep -l 'openclaw\\|claude_harness'" in line:
        line = line.replace('command: "', "command: '")
        line = line.rstrip('\n').rstrip('"') + "'\n"
...
print('YAML_OK')
print('FIX_SUMMARY:')
print('  - Fixed illegal escape chars in command fields: changed double quotes to single quotes')
