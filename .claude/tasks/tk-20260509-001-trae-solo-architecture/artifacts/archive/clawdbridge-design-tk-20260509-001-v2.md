...
> 版本: v2.0 | 2026-05-09 | 阶段: Stage 4 (Design)
...
> 变更: v1→v2 补充 14 个缺口（G01 MsgRouter, G02 SessionSearch, G03-G08 DAO/Service 方法签名, G09-G14 接口/配置签名）
...
const MainTabNavigator = () => (
  <Tab.Navigator>
    ...
    <Tab.Screen name="Profile" component={ProfileTab} options={{ tabBarLabel: '我的' }} />
  </Tab.Navigator>
);
```

### 2.2 SessionSearch 组件

```typescript
// mobile/src/components/session-search.tsx
interface SessionSearchProps {
  onSearch: (query: string) => void;   // 实时搜索回调
  placeholder?: string;                // 默认 "搜索对话..."
}
// 搜索范围: 对话标题 + 首条消息内容前 100 字
// 实现: 本地 SQLite LIKE 查询，防抖 300ms
```
```
...
```typescript
// BP15 — Express JWT 验证中间件 (bridge-server/src/jwt-verifier.ts)
type JWTMiddleware = (req: Request, res: Response, next: NextFunction) => void;
// 内部逻辑: extract Bearer token → HS256 verify → req.user = { userId, deviceId } | 401

// MP11 — WS 接收器 (mobile/src/services/ws-receiver.ts)
interface WSReceiver {
  onMessage(rawJson: string): void;
  // 内部流程: JSON.parse → 校验 id/type/sessionId/seq/ts → dispatch(router, type, payload)
}

// MP27 — 审批处理 Hook (mobile/src/hooks/use-approval-handler.ts)
interface UseApprovalHandlerReturn {
  request: ApprovalRequestPayload | null;   // 当前待审批请求 (null=无)
  respond: (decision: 'approved' | 'rejected', scope: 'once' | 'session') => void;
  dismiss: () => void;                      // 手动关闭（等同于 reject）
  isExpired: boolean;                       // 是否已超时
}
// 内部逻辑:
// - 收到 approval_request → set request + 启动 60s timer
// - 用户点允许/拒绝 → send approval_response → set request=null
// - 60s timeout → setIsExpired(true) → auto dismiss (不发 response)

// MP29 — 会话级审批范围 Hook (mobile/src/hooks/use-approval-session-scope.ts)
interface UseSessionScopeReturn {
  isSessionScope: boolean;
  toggleSessionScope: () => void;           // 勾选/取消
}
// scope='session' 时发送 approval_response 附带 scope 字段
// Bridge 端 ApprovalWhitelist 记录，后续同操作跳过等待
```
```
...
### 9.4 SecureStore 封装 API (Mobile 端)

```typescript
// mobile/src/services/secure-store.ts
...
// 底层: expo-secure-store (iOS Keychain / Android Keystore)
// Token 刷新时调用 storeTokens 更新，注销时调用 clearTokens
```
...
### 5.6 MsgRouter 路由表（Mobile 端）

接收到的 WS 消息按 type 分发到以下 handler：

```typescript
// mobile/src/services/msg-router.ts 路由表
const ROUTE_TABLE: Record<MessageType, Handler> = {
  'session_sync':      (payload) => { sessionStore.hydrate(payload.sessions); chatStore.setPendingApprovals(payload.pendingApprovals); },
  'assistant_stream':  (payload) => { chatStore.receiveStream(payload.delta, payload.done, payload.messageId, payload.sessionId); },
  'tool_invocation':   (payload) => { chatStore.addToolCall(payload.sessionId, payload as ToolInvocationPayload); },
  'tool_result':       (payload) => { chatStore.markToolResult(payload.sessionId, payload.callId, payload as ToolResultPayload); },
  'approval_request':  (payload) => { chatStore.addApprovalRequest(payload.sessionId, payload as ApprovalRequestPayload); },
  'session_state':     (payload) => { chatStore.updateSessionStatus(payload.sessionId, payload.status); },
  'error':             (payload) => { chatStore.addError(payload.sessionId, payload as BridgeErrorPayload); },
  'pong':              (_payload) => { /* 心跳应答，仅更新 lastHeartbeat */ },
};

// 路由入口
function dispatch(sessionId: string, type: MessageType, payload: unknown): void {
  const handler = ROUTE_TABLE[type];
  if (handler) handler(payload);
}
```

### 5.7 WSSender API（Mobile 端）

```typescript
// mobile/src/services/ws-sender.ts
interface WSSender {
  // 发送消息，自动分配 seq
  send(sessionId: string, msg: { type: MessageType; payload: unknown }): void;

  // 获取下一条待发送消息的 seq（自增）
  nextSeq(sessionId: string): number;

  // Bridge ACK 确认 seq 已收到，标记消息为 sent
  handleAck(sessionId: string, ackSeq: number): void;

  // 补发 [lastAckSeq+1 ... currentSeq] 的未确认消息
  resendPending(sessionId: string): void;

  // 获取未确认消息队列
  getPendingSeqRange(sessionId: string): { from: number; to: number } | null;
}
```
...
```typescript
// ── SessionDAO (mobile/src/db/session-dao.ts) ──
interface SessionDAO {
  create(title: string, deviceId: string, deviceName: string): Promise<Session>;
  list(filter?: { archived?: boolean }): Promise<Session[]>;
  getById(id: string): Promise<Session | null>;
  updateLastMessage(id: string, timestamp: number): Promise<void>;
  updateStatus(id: string, status: 'idle' | 'running' | 'error'): Promise<void>;
  incrementUnread(id: string): Promise<void>;
  clearUnread(id: string): Promise<void>;
  incrementPendingApprovals(id: string): Promise<void>;
  decrementPendingApprovals(id: string): Promise<void>;
  archive(id: string): Promise<void>;
}

// ── MessageDAO (mobile/src/db/message-dao.ts) ──
interface MessageDAO {
  insert(msg: Message): Promise<void>;
  getPage(sessionId: string, beforeTimestamp: number, limit: number): Promise<{ messages: Message[]; hasMore: boolean }>;
  getLastSeq(sessionId: string): Promise<number>;
  updateStatus(id: string, status: 'sent' | 'failed'): Promise<void>;
  getBySeqRange(sessionId: string, fromSeq: number, toSeq: number): Promise<Message[]>;
}

// ── ApprovalDAO (mobile/src/db/approval-dao.ts) ──
interface ApprovalDAO {
  insert(approval: ApprovalRecord): Promise<void>;
  getBySession(sessionId: string, limit?: number): Promise<ApprovalRecord[]>;
  updateDecision(id: string, decision: 'approved' | 'rejected' | 'auto_rejected'): Promise<void>;
}

// ── KVStoreDAO (mobile/src/db/kv-store-dao.ts) ──
...
```json
{
  "dependencies": {
    ...
    "zustand": "^5.x",
    "react-native-websocket": "^1.x",
    "expo-sqlite": "~15.x",
    "expo-secure-store": "~14.x",
    "expo-auth-session": "~6.x",
    "expo-speech-recognition": "~1.x",
...
│   └─ jwtVerifier middleware: (req, res, next) →   // Express 中间件
│       读取 req.headers.authorization → 提取 Bearer <token>
│       → verifyToken → 成功: req.user = { userId, deviceId } + next()
│                      → 失败: res.status(401).json({ error: 'invalid_token' })
│
├─ DeviceManager
