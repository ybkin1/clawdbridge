...
5. blocked 后静默推进
6. **Auditor verdict 篡改** — 主线程不得以任何方式覆盖或伪造 Auditor 的 receipt-auditor.yaml  verdict。检测到篡改时，task 自动进入 `blocked` 状态并标记 `tamper_detected`。（见 `review-gates-contract.md` §18.3）
