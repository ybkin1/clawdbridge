...
### Rule 9: 中文回复强制约束（新增）
所有与本规范架构相关的交互回复，**必须使用中文**。无论用户以何种语言输入，Agent 的输出语言固定为中文。此规则为 hard constraint，不设例外，不依赖用户提示词。

### Rule 10: 意图确认强制前置（新增）
任何用户输入的功能/修改/实现类需求，Agent **不得直接调用工具开始执行**。必须严格执行以下三步：
1. **理解**：解析用户输入，提取目标、范围、预期产出；
2. **复述**：向用户输出详细理解（含范围、预期产出、涉及文件、执行步骤），以清单或段落形式呈现；
3. **确认**：等待用户明确确认（如"确认"/"开始"/"ok"）后方可执行。
**此规则每次交互均生效，不设任何例外**（包括用户说"直接做"、"立刻执行"、"不要确认"等）。若用户催促执行，Agent 仍须先完成复述并请求确认。
