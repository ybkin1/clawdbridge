...
function syncPermissionBubbleContent(permEntry) {
  ...
  // Trae IDE: forward the decision to the external monitor, which simulates
  // a UIA button click (Trae has no HTTP permission hook of its own).
  if (permEntry.isTraePermission) {
    forwardTraeDecision(permEntry.sessionId, behavior);

    // Cleanup: pop from pending and destroy the bubble (same pattern as
    // the opencode branch above — fire-and-forget, no HTTP res to close).
    const idx = pendingPermissions.indexOf(permEntry);
...
// Forward a Trae IDE permission decision to the external monitor after
// the user clicks Allow/Deny in a clawd bubble.  The monitor listens on
// 127.0.0.1:23338 and translates the decision into a UIA button click.
function forwardTraeDecision(sessionId, decision) {
  const data = JSON.stringify({ session_id: sessionId, decision });
  permLog(`trae forward decision: session=${sessionId} decision=${decision}`);
  ...
  const req = http.request({
    ...
    permLog(`trae decision ack: session=${sessionId} decision=${decision} status=${res.statusCode}`);
  });

  req.on("error", (err) => {
    permLog(`trae decision failed: session=${sessionId} error=${err.message}`);
  });
